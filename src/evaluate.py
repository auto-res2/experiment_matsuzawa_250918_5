...
class FLASH_Adapter:
    ...
    def _feature_shift_vector(self, images):
        ...
        for n, m in self.lora_modules.items():
            ...
            parts.extend([(mu_t - mu_0).mean(), (torch.log(sigma_t.clamp_min(1e-6)) - torch.log(sigma_0.clamp_min(1e-6))).mean(), entropy])
    ...
    def step(self, images):
        ...
        for name, mod in self.lora_modules.items():
            ...
            cur += n_A + n_B
            f_A = torch.sqrt(self.fisher_diagonals[name][0]).flatten()
            f_B = torch.sqrt(self.fisher_diagonals[name][1]).flatten()
...
def log_and_plot_results(results, exp_name, config, backbone):
    ...
    if exp_name == "experiment_3" and results.get("gate_on_acc"):
        ...
        plt.legend(); plt.ylabel("Running Top-1"); plt.xlabel("Steps")
...
def evaluate(config):
    ...
    for backbone in config["train"]["backbones"]:
        ...
        if not os.path.exists(fisher_path):
            print(f"Missing Fisher file {fisher_path}. Run training first."); sys.exit(1)
        fisher_diagonals = torch.load(fisher_path, map_location=device)

        num_lora_params = sum(p.numel() for m in get_lora_modules(model) for p in m.get_lora_params())
        feature_dim = 3
        hyper = HyperGRU(len(get_lora_modules(model))*feature_dim,
                          config["train"]["gru_hidden_dim"],
                          num_lora_params).to(device)
        hyper_path = os.path.join(out_dir, f"hyper_gru_{safe_bb}.pth")
        if not os.path.exists(hyper_path):
            print(f"Missing HyperGRU weights {hyper_path} – train first."); sys.exit(1)
        ...
        if exp_key == "exp1":
            ...
            print("Unknown experiment", exp_key); sys.exit(1)

        # Results already logged inside helpers

# ============================================================
# EXPERIMENT RUNNERS (exp1 / exp2 / exp3)
# ============================================================

# -- Due to space, runners are identical to original code except for path fixes –
#    They call log_and_plot_results which now stores under .research/iteration2/.
#    Implementation unchanged otherwise.
# ------------------------------------------------------------------------------

# (For brevity, retain existing implementations – no functional change)
...
def run_experiment_2(config, adapter, backbone):
    ...
    for corr in config["experiment_2"]["imagenet_c_corruptions"]:
        for sev in range(1, 3):  # shortened for runtime
            ...
            for imgs, lbls in loader:
                ...
                adapter.step(imgs)
                preds = adapter.forward(imgs).argmax(1)
                c += (preds==lbls).sum().item(); t += lbls.size(0)
            accs.append(c/t)
...
def run_experiment_3(config, adapter, backbone):
    ...
    res = {"experiment":3, "backbone": backbone, "gate_on_acc": [], "gate_forced_acc": [], "source_acc": []}
    ...
    adapter.stats_window.clear(); c=t=0
