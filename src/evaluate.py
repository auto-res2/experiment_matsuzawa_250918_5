...
class NVMLMonitor:
    ...
    def stop(self):
        if not self.handle: return {'energy_joules': 0, 'peak_mem_gb': 0}
...
def calculate_dirichlet_energy(x, edge_index):
    laplacian_edge_index, laplacian_edge_weight = get_laplacian(edge_index, normalization='sym', num_nodes=x.size(0))
    L = to_scipy_sparse_matrix(laplacian_edge_index, laplacian_edge_weight, num_nodes=x.size(0))
    energy = 0
...
def calculate_js_divergence(p, q, base=2.):
...
def calculate_attention_sign_confusion(model, data):
    ...
    model.eval()
    with torch.no_grad():
        _, (edge_index, attn_scores) = model.layers[0].attn(data.x, data.edge_index, return_attention_weights=True)
        # This is a simplification. Real attention retrieval is complex.
        # Assuming attn_scores aligns with data.edge_index
...
def generate_plots(results, output_dir, experiment_id):
    if experiment_id == 3:
        # Plot Dirichlet Energy vs. Layer
        plt.figure(figsize=(10, 6))
        for model_name, metrics in results.items():
            if 'dirichlet_energy_per_layer' in metrics:
                layers = range(len(metrics['dirichlet_energy_per_layer']))
                plt.plot(layers, metrics['dirichlet_energy_per_layer'], marker='o', label=model_name)
        plt.xlabel('Layer')
        plt.ylabel('Dirichlet Energy')
        plt.title('Dirichlet Energy vs. Model Depth')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(output_dir, 'dirichlet_energy_vs_depth.pdf'))
        plt.close()
...
def run_evaluation(config, experiment_id, data_snapshots):
    ...
    for model_config in exp_config['models_and_baselines']:
        ...
        for seed in config['global_settings']['seeds']:
            model_path = os.path.join(config['global_settings']['output_dir'], f'exp{experiment_id}', 'models', f"{model_name}_seed{seed}.pt")
            if not os.path.exists(model_path):
                print(f"Warning: Model checkpoint not found for {model_name} seed {seed}, skipping.", file=sys.stderr)
                continue
            ...
            if experiment_id == 3 and hasattr(model, 'layers'):
                ...
                for layer in model.layers:
                    ...
                    if hasattr(layer, 'out_channels') and h.size(1) == layer.out_channels * layer.heads:
        ...
        for metric, values in avg_metrics.items():
            if isinstance(values[0], list):
                ...
                model_results[f'{metric}_mean'] = np.mean(values)
                model_results[f'{metric}_std'] = np.std(values)

        results[model_name] = model_results

    # Save and print results
    results_dir = os.path.join(config['global_settings']['output_dir'], f'exp{experiment_id}', 'metrics')
    ...
    # Generate plots
    figures_dir = os.path.join(config['global_settings']['output_dir'], f'exp{experiment_id}', 'figures')
    os.makedirs(figures_dir, exist_ok=True)
    generate_plots(results, figures_dir, experiment_id)
